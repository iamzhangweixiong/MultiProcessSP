...
import com.wilson.multiprocesssp.BuildConfig;
...
public abstract class BaseSpProvider extends ContentProvider {
    ...
    private Map<String, ISharedPreference> mConfigMap;

    static Uri convertUri(String param) {
        return Uri.parse(getContentProviderUri() + "/" + param);
    }

    public static String getContentProviderUri() {
        if (CONTENT_URI == null) {
            CONTENT_URI = ContentResolver.SCHEME_CONTENT + "://" + BuildConfig.APPLICATION_ID + ".sp_provider";
        }
        return CONTENT_URI;
    }
    ...
    protected abstract void updateConfigMap(Map<String, ISharedPreference> configMap);
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        ISharedPreference appConfig = mConfigMap.get(configName);
        if (appConfig == null) {
            return null;
        }
        ...
        if (nType == TYPE_BOOLEAN) {
            ...
            res = URLEncoder.encode(res);
        } else if (nType == TYPE_FLOAT) {
            res += appConfig.getFloatValue(values.getAsString(EXTRA_KEY),
    ...
    public int update(Uri uri, ContentValues values, String selection,
        ...
        if (nType == TYPE_BOOLEAN) {
            ...
            appConfig.setFloatValue(values.getAsString(EXTRA_KEY),
                    values.getAsFloat(EXTRA_VALUE));
        } else if (nType == TYPE_INT) {
